...
app.view('announce_modal_submit', async ({ ack, body, view, client, logger }) => {
  ...
  if (scheduleAt) {
    ...
    // Channels (from multi_conversations_select)
    ...
    for (const gv of groupValues) {
      try {
        ...
        logger.error('Error resolving announce user group for schedule:', err);
      }
    }
    ...
    for (const channelId of selectedChannels) {
      try {
        ...
        logger.error('Error resolving announce channel for schedule:', err);
      }
    }
    ...
    if (recipients.length === 0) {
      await client.chat.postMessage({
        channel: senderId,
        text: '❌ Couldn\'t resolve any recipients.',
      });
      return;
    }
  ...
  if (scheduleAt) {
    ...
    for (const gv of groupValues) {
      try {
        ...
        logger.error('Error resolving pulse user group for schedule:', err);
      }
    }
    ...
    for (const channelId of selectedChannels) {
      try {
        ...
        logger.error('Error resolving pulse channel for schedule:', err);
      }
    }
